...
def get_peft_model_state_dict(model, state_dict=None):
    ...
    elif model.peft_config.peft_type == PeftType.LILY:
        to_return = {k: state_dict[k] for k in state_dict if "lily" in k}
...
def set_peft_model_state_dict(model, peft_model_state_dict):
    ...
    if model.peft_config.peft_type != PeftType.LORA and model.peft_config.peft_type != PeftType.BOTTLENECK and model.peft_config.peft_type != PeftType.LILY:
